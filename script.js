...
setAlarm.addEventListener('click',function(){myFunc();});

function myFunc(){

    const time = document.getElementById("inputAlarm").value;
    if(time=="")
    alert("Enter a time to set alarm");
...
function appendAlarm(){
    ...
    list.forEach((alarm)=>{
        ...
        setTimeout(function(){ringAlarm();},timeDifference);
...
function ringAlarm(){
    ...
    document.getElementById('container').style.setProperty('filter','blur(50px)');
    document.getElementById('alert').innerHTML=`${document.getElementById('alert-text').value==""?"Hey I'm here to remind you":document.getElementById('alert-text').value}`;
    ...
    document.getElementById("okay").addEventListener("click", function(){
        ...
        document.getElementById('container').style.setProperty('filter','blur(0px)');
    });
